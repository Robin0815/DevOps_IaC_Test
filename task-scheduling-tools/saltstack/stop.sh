...
print_header "🛑 Stopping SaltStack"
echo "====================="

# Check if containers are running
if ! docker-compose ps | grep -q "Up"; then
    print_warning "No SaltStack containers are currently running"
    exit 0
fi
...
# Stop containers
print_info "Stopping SaltStack containers..."
if docker-compose down; then
    print_status "SaltStack containers stopped"
else
...
if [ "$CLEANUP_VOLUMES" = true ]; then
    ...
    if [ "$FORCE" = true ]; then
        ...
        echo -e "${YELLOW}⚠️  This will remove all SaltStack data including configurations, keys, and logs.${NC}"
