...
if ! docker-compose ps | grep -q "Up"; then
    print_warning "No Jenkins containers are currently running"
    exit 0
fi
...
# Stop containers
print_info "Stopping Jenkins containers..."
if docker-compose down; then
    print_status "Jenkins containers stopped"
else
...
if [ "$CLEANUP_VOLUMES" = true ]; then
    ...
    if [ "$FORCE" = true ]; then
        ...
        echo -e "${YELLOW}⚠️  This will remove all Jenkins data including jobs, builds, and configurations.${NC}"
