...
if ! docker-compose ps | grep -q "Up"; then
    print_warning "No Airflow containers are currently running"
    exit 0
fi
...
if docker-compose down; then
    print_status "Airflow containers stopped"
else
...
if [ "$CLEANUP_VOLUMES" = true ]; then
    ...
    if [ "$FORCE" = true ]; then
        ...
        echo -e "${YELLOW}⚠️  This will remove all Airflow data including DAGs, logs, and database.${NC}"
