...
print_header "🛑 Stopping StackStorm Workflow Engine"
echo "======================================"

# Check if containers are running
if ! docker-compose ps | grep -q "Up"; then
    print_warning "No StackStorm containers are currently running"
    exit 0
fi
...
# Stop containers
print_info "Stopping StackStorm containers..."
if docker-compose down; then
    print_status "StackStorm containers stopped"
else
...
if [ "$CLEANUP_VOLUMES" = true ]; then
    ...
    if [ "$FORCE" = true ]; then
        ...
        echo -e "${YELLOW}⚠️  This will remove all StackStorm data including workflows, logs, and configurations.${NC}"
